...
int calculTour(struct Voiture *v, int fCourse){
  int tempsPit, tour = 0.0, section;
  if (out() == 1) {
    v->out = 1;
  }
  else{
...
void courseFinale(struct MemoirePartagee *tabVoitures, int i){
  ...
  while (v.nbTour < tabVoitures->nbTourAFaire && v.out == 0) { //tabVoitures->nbTourAFaire
    if (v.pit == 0){
      calculTour(pointeurV, 1);
      tabVoitures->tableauV[i] = v;
      msleep(1000);
    }
    else{
      calculTour(pointeurV, 0);
      tabVoitures->tableauV[i] = v;
      msleep(1000);
    }
  }
...
void essaisEtQualifications(struct MemoirePartagee *tabVoitures, int i, int tempsTotal){
  struct Voiture v;
  struct Voiture *pointeurV = &v;
  ...
  while (tempsEnCours < tempsTotal && v.out == 0) {
    tempsEnCours += calculTour(pointeurV, 0);
    tabVoitures->tableauV[i] = v;
...
int main(int argc, char *argv[]){
  ...
  int shmid = shmget(IPC_PRIVATE, sizeof(tabVoitures), IPC_CREAT | 0666);
